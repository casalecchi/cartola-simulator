...
class Simulation():
    ...
    def __init__(self, data_list:List['instance.Data'], strat_list: List[List[int]], solver: base_solver.Solver):
    ...
    def solve(self):
        for r in range(1,len(self.instance)):
            ...
            squad,cap = self.chosen_solver.solve(self.instance[r],self.strat_list,self.money, self.proficiency)
            if len(squad) < 12: 
            ...
            if r < len(self.instance)-1: 
                self.money = self.UpdateMoney(self.instance[r],self.instance[r+1],squad,self.money)
            print(str(round(self.point,2))+" "+str(round(self.money,2))+" "+str(round(self.money-aux_money,2)))
            print("")
