...
class LSM(base_solver.Solver):
    ...
    def solve(self, data_inst: instance.Data, strat_list:List[List[int]], money: Double, proficiency):
        with localsolver.LocalSolver() as ls:
            squad = []

            model = ls.model

            y = [[model.bool() for j in range(len(data_inst.name))] for i in range(len(data_inst.posUnique))]
            x = [model.bool() for e in range(len(strat_list))]

            knapsack_weight = model.sum( model.sum( data_inst.cost[j] * y[i][j] for j in range(len(data_inst.name)) ) for i in range(len(data_inst.posUnique)) )
            ...
            for i in range(len(data_inst.posUnique)):
                ...
                obj1 +=  model.sum( mean(proficiency[data_inst.name[j]]) * y[i][j] for j in range(len(data_inst.name))) 
            ...
            model.constraint(teamsize == model.sum(sum(strat_list[e])*x[e]  for e in range(len(strat_list))))
            ...
            for j in squad:
                if cap == j:
                    print(data_inst.name[j]+" "+ data_inst.position[j] +" "+str(data_inst.score[j])+" - Capitão")
                else:
                    print(data_inst.name[j]+" "+ data_inst.position[j] +" "+str(data_inst.score[j]))
